...
import {
  ...
  getActivityLogStats,
  markActivityLogRead,
  markActivityLogsBatch,
  markAllActivityLogsRead,
} from "../../api/activity_logs/getActivityLogs";
...
export default function ActivityLogs() {
    ...
    try {
      ...
      if (result.success) {
        ...
        setStats({
          ...
          borrowed:
            byAction.find((a) => a.action === "BOOK_BORROWED")?.count || 0,
          returned:
            byAction.find((a) => a.action === "BOOK_RETURNED")?.count || 0,
          penalties_paid:
            byAction.find((a) => a.action === "PENALTY_PAID")?.count || 0,
        });
      }
    } catch (err) {
  ...
  const getActionIcon = (action) => {
    switch (action) {
      ...
      case "BOOK_RETURNED":
        return <FaUndo className="text-success" />;
      case "PENALTY_PAID":
        return <FaMoneyBillWave className="text-warning" />;
      default:
        return <FaHistory className="text-muted" />;
    }
  };
  ...
  const handleMarkRead = async (log) => {
    try {
      ...
      setLogs((prev) =>
        prev.map((l) =>
          l.activity_log_id === log.activity_log_id
            ? {
                ...l,
                is_read: 1,
                read_at: new Date().toISOString(),
                read_by_admin_id: adminId,
              }
            : l
        )
      );
  ...
  const handleMarkVisibleAsRead = async () => {
    try {
      ...
      setLogs((prev) =>
        prev.map((l) =>
          ids.includes(l.activity_log_id)
            ? {
                ...l,
                is_read: 1,
                read_at: new Date().toISOString(),
      ...
      <div className="d-flex justify-content-between align-items-center mb-4">
        <div>
          ...
          <p className="text-muted mb-0 small">
            Monitor system activities | Notifications
          </p>
        </div>
        <div />
      </div>
      ...
      <div className="d-flex justify-content-end align-items-center mb-3 gap-2">
        ...
        <button
          ...
          onClick={() => {
            ...
            if (
              !window.confirm(
                "Mark ALL unread activity logs as read? This will affect all records in the database."
              )
            )
              return;
            handleMarkAllRead();
          ...
          <div className="table-responsive">
            <table className="table table-hover align-middle mb-0">
              <thead className="bg-light">
                <tr>
                  <th className="px-4 py-3 border-0" style={{ width: "180px" }}>
                    Timestamp
                  </th>
                  <th className="px-4 py-3 border-0">User</th>
                  <th className="px-4 py-3 border-0" style={{ width: "150px" }}>
                    Action
                  </th>
                  <th className="px-4 py-3 border-0">Details</th>
                  <th className="px-4 py-3 border-0" style={{ width: "100px" }}>
                    Status
                  </th>
                  <th
                      ...
                      <td className="px-4 py-3 text-center">
                        <div className="d-flex gap-2 justify-content-center">
                          <button
                            ...
                            title={
                              log.is_read ? "Already read" : "Mark as read"
                            }
              ...
              <div className="modal-body">
                <div className="row g-3">
                  ...
                  <div className="col-md-6">
                    ...
                    <div className="fw-semibold">
                      {selectedLog.position || "N/A"}
                    </div>
                  </div>
                  <div className="col-md-6">
                    <label className="text-muted small mb-1">Department</label>
                    <div className="fw-semibold">
                      {selectedLog.department_acronym || "N/A"}
                    </div>
                  </div>
                  <div className="col-md-6">
